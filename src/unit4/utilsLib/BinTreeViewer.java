...
class BinTreeViewer
{
  ...
  private BinTreeViewer(JFrame f, TreePanel p,Object t) throws Exception
  {
   this.f = f;
   this.panel = p;
   this.tree = clone(t);
...
//    System.err.println(constructors[i]);
  ...
  {  
   ...
   try
     ...
     try
     {
      TreePanel panel = new TreePanel(tree,20);
//System.err.println("1");
      BinTreeViewer td = new BinTreeViewer(f,panel,tree);
...
//System.err.println("5");
  ...
  {
     TreePanel panel = new TreePanel(t,nodeSize);
     return(panel);
  }
...
class TreePanel extends JPanel 
{
  ...
  private TVNode[] nodes = new TVNode[MAXNODES];
  private LinkB[] links = new LinkB[MAXNODES-1];
  private int numNodes = 0;
  private int numLinkBs = 0;
  

   public TreePanel(Object t, int nodeSize) throws Exception
   {
    super();
 ...
 public int buildTree(Object t, int depth) throws Exception
   ...
   Object right = t.getClass().getMethod("getRight",(Class[])null).invoke(t, (Object[])null);
     ...
     if (left != null) 
     {
      ...
      links[numLinkBs] = new LinkB( u, numNodes );
      numLinkBs++;
     }
    
     int index = numNodes;
     nodes[numNodes] = new TVNode(info.toString(), numNodes, depth );
     numNodes++;
     
     if (right != null) 
     {
      int u = buildTree(right, depth+1);
      links[numLinkBs] = new LinkB( u, index );
      numLinkBs++;
     }
     return index;
    }
...
class TVNode
{
  ...
  public TVNode( String keyVal, int xVal, int yVal )
  {
    key = keyVal;
    x = xVal;
    y = yVal;
...
class LinkB
{
  ...
  public LinkB( int uVal, int vVal )
  {
    u = uVal;
    v = vVal;
  }
